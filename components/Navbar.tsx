...
export function Navbar() {
  // Simulated auth state - default to false per user request
  ...
  return (
    <nav className="fixed top-4 left-0 right-0 z-50 px-8 lg:px-16 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-6">
        {!isLoggedIn ? (
          <>
            <button className="text-sm font-body font-medium text-white/60 hover:text-white transition-colors">
              Login
            </button>
            <button className="bg-white text-black rounded-full px-5 py-1.5 text-sm font-body font-bold flex items-center gap-1 hover:bg-white/90 transition-all hover:shadow-[0_0_15px_rgba(255,255,255,0.3)]">
              Sign in
            </button>
          </>
        ) : (
          <button className="bg-white text-black rounded-full px-5 py-1.5 text-sm font-body font-bold flex items-center gap-1 hover:bg-white/90 transition-all">
            Dashboard
