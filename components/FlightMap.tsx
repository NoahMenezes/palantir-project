...
const AIRPLANE_SVG = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"><path d="M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z"/></svg>');
...
export function FlightMap() {
  ...
  const layers = [
    new IconLayer({
      ...
      autoHighlight: true,
      onClick: ({object}) => {
        if (object) setSelectedFlight(object);
      }
    })
  ];

  return (
    <div className="w-full h-full relative group rounded-2xl overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(0,229,255,0.05)] bg-[#0A0A0C] flex flex-row">
      ...
      <div className="flex-1 relative h-full">
        <DeckGL
          ...
          layers={layers}
          getTooltip={({object}) => object && !selectedFlight ? `${object.callsign}\n${object.country}` : null}
          style={{ position: 'absolute', width: '100%', height: '100%' }}
        >
          {/* reuseMaps perfectly aligns Maplibre and DeckGL, solving the slipping/dragging bug entirely */}
        ...
        {/* Top Left Indicator */}
        <div className="absolute top-4 left-4 bg-black/80 backdrop-blur-md px-4 py-2 rounded-lg border border-white/10 flex flex-col gap-1 pointer-events-none z-10">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-mono text-[#00E5FF] uppercase tracking-widest font-bold">Live ADS-B Radar</span>
          </div>
          <span className="text-[10px] font-mono text-white/50 pl-5">
            {loading ? "Establishing Uplink..." : error ? "Connection Failed" : `Tracking ${flights.length.toLocaleString()} Active Flights`}
          </span>
        </div>

        {/* Bottom Panel Link */}
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 pointer-events-auto">
          <Link href="/flights" className="bg-[#00E5FF]/10 hover:bg-[#00E5FF]/20 text-[#00E5FF] backdrop-blur-md px-6 py-3 rounded-full border border-[#00E5FF]/30 transition-all font-mono text-xs uppercase tracking-widest flex items-center gap-2 shadow-[0_0_20px_rgba(0,229,255,0.1)] hover:shadow-[0_0_30px_rgba(0,229,255,0.2)]">
        ...
        {/* Altitude Legend */}
        <div className="absolute bottom-4 right-4 bg-black/80 backdrop-blur-md p-3 rounded-lg border border-white/10 z-10 pointer-events-none hidden md:block">
          <span className="text-[9px] font-mono text-white/50 uppercase tracking-widest mb-2 block">Altitude (ft)</span>
          ...
          <div className="p-4 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm font-mono text-white">
                    {selectedFlight.velocity ? Math.round(selectedFlight.velocity * 1.94384) : "N/A"} kt
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 uppercase font-mono">Baro. Altitude</p>
                  <p className="text-sm font-mono text-white">
                    {selectedFlight.baro_altitude ? Math.round(selectedFlight.baro_altitude * 3.28084).toLocaleString() : "N/A"} ft
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 uppercase font-mono">Geo. Altitude</p>
                  <p className="text-sm font-mono text-white">
                    {selectedFlight.geo_altitude ? Math.round(selectedFlight.geo_altitude * 3.28084).toLocaleString() : "N/A"} ft
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 uppercase font-mono">Vert. Rate</p>
                  <p className="text-sm font-mono text-white">
                    {selectedFlight.vertical_rate ? Math.round(selectedFlight.vertical_rate * 196.85) : 0} ft/min
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 uppercase font-mono">Track (Heading)</p>
                  <p className="text-sm font-mono text-white">{Math.round(selectedFlight.track)}&deg;</p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 uppercase font-mono">Position</p>
                  <p className="text-[11px] font-mono text-[#00E5FF]">{selectedFlight.lat?.toFixed(3)}&deg;, {selectedFlight.lng?.toFixed(3)}&deg;</p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-[10px] text-white/40 uppercase font-mono">Source</p>
                  <p className="text-sm font-mono text-white">
                    {selectedFlight.position_source === 0 ? "ADS-B" : selectedFlight.position_source === 1 ? "ASTERIX" : selectedFlight.position_source === 2 ? "MLAT" : "FLARM"}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-white/40 uppercase font-mono">Last Contact</p>
                  <p className="text-sm font-mono text-white">
                    {selectedFlight.last_contact ? `${Math.floor(Date.now()/1000 - selectedFlight.last_contact)}s ago` : "N/A"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 border border-white/10 rounded-lg p-4 bg-white/[0.02]">
              <p className="text-[10px] text-white/60 font-body mb-3">Live data telemetry automatically refreshes.</p>
